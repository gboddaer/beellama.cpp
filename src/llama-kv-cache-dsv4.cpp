...
bool llama_kv_cache_dsv4::seq_rm_cell(llama_seq_id seq_id, uint32_t cell_idx) {
    // DSV4 doesn't have individual cells like standard KV cache
    GGML_UNUSED(seq_id); GGML_UNUSED(cell_idx);
    return false;
}

int llama_kv_cache_dsv4::cells_at_pos(llama_seq_id seq_id, llama_pos pos, uint32_t * cell_indices, int n_max) {
    // DSV4 doesn't support cell-based position queries
    GGML_UNUSED(seq_id); GGML_UNUSED(pos); GGML_UNUSED(cell_indices); GGML_UNUSED(n_max);
    return 0;
}
...
void llama_kv_cache_dsv4::seq_cp_recurrent(llama_seq_id, llama_seq_id, llama_pos, llama_pos) {
    // DSV4 has no recurrent state
}
