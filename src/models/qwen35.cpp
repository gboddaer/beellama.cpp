...
llama_model_qwen35::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // Copies the prompt-suffix slice of each captured layer output into the per-slot
        // prefill_gpu buffers so the drafter's cross-attention sees the correct suffix span.
