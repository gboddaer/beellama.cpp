...
extern "C" {
    ...
    struct llama_model_params {
        ...
        // DFlash: output device override
        ggml_backend_dev_t output_device; // device for output tensors (nullptr = auto)
    ...
    struct llama_context_params {
        ...
        // DFlash drafter: cross-attention context window (K cache rows the drafter can attend to).
        // 0 = unlimited (full target sequence). Larger values increase drafter KV cache but reduce
        // wasted compute on irrelevant context. PER_SLOT_CTX is the compile-time default.
    ...
    // DFlash compile-time constants
    enum {
        LLAMA_DFLASH_MAX_VERIFY_TOKENS = 25, // must be >= draft_max + 1
        LLAMA_DFLASH_MAX_SLOTS         = 8,
        LLAMA_DFLASH_PER_SLOT_CTX      = 512, // default cross-attention context per slot
    };
...
// DFlash API
    ...
    LLAMA_API void   llama_dflash_cross_ring_gpu_wait_interleave(void * handle);
